...
if($src!= '')
{	
	//$srcFileName ='sites/literature.ironikopoulou.gr/files/imagefield_thumbs/images/'.$src;	
...
//	echo htmlentities($srcFileName);
	...
	*/
	...
	$nodeTitleWithHTMLEntities = preg_replace('/\"/', '&#34;', $row->node_title);
	
	print '<a href="'.$newSrc.'" rel="lightbox[gall-'.$viewUrl[1].']['.preg_replace('/\d{4}-\d{2}-\d{2}./', '', $nodeTitleWithHTMLEntities).']" >'.
			$imgOutput.
	  '</a>';
}
?>
